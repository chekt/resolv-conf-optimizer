...
func checkNameServerSpeed(server string, target string) (duration time.Duration, totalRecord int, err error) {
	c := dns.Client{}
	m := dns.Msg{}
	m.SetQuestion(target+".", dns.TypeA)
	res, duration, err := c.Exchange(&m, server)
	if err != nil {
		return duration, totalRecord, err
	}
	totalRecord = len(res.Answer)
	if totalRecord == 0 {
...
func main() {
	...
	// 2. Get nameserver from resolv.conf
	serverInfoList, err := getNameServers()
	if err != nil {
		log.Fatal(err)
	}

	// 3. Check nameserver latency
	for i, serverInfo := range serverInfoList {
		server := serverInfo.Server
		duration, totalRecord, err := checkNameServerSpeed(server, target)
		if err != nil {
			message := fmt.Sprintf("server: %v, records: %v, speed: %v, error: %v", server, totalRecord, duration, err)
			log.Println(message)
		} else {
			message := fmt.Sprintf("server: %v, records: %v, speed: %v, error: %v", server, totalRecord, duration, err)
			log.Println(message)
